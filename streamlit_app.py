...
with tab1:
    ...
    # Approximate state centroids, for two-letter state abbreviation labels on the map
    STATE_CENTROIDS = {
    ...
    state_df = pd.DataFrame(
        [{"state": k, "lat": v[0], "lon": v[1]} for k, v in STATE_CENTROIDS.items()]
    )

    fig_map = go.Figure()

    # State abbreviation labels — added FIRST so airport markers render on top of them
    fig_map.add_trace(go.Scattergeo(
        lon=state_df["lon"], lat=state_df["lat"],
        text=state_df["state"], mode="text",
    ...
    fig_map.add_trace(go.Scattergeo(
        ...
        marker=dict(
            ...
            colorbar=dict(
                title=dict(text="Delay<br>Rate (%)", font=dict(color="#1e293b", family=FONT_FAMILY)),
                tickfont=dict(color="#1e293b", family=FONT_FAMILY),
    ...
    fig_map.update_layout(
        ...
        height=460, font=dict(family=FONT_FAMILY, color="#1e293b"),
